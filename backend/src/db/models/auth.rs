...
fn validate_email(email: &str) -> Result<(), ValidationError> {
    ...
    let at_index = email
        .find('@')
        .ok_or_else(|| ValidationError::new("email_missing_at_sign"))?;
    ...
    if invalid_special_chars_regex.is_match(domain_part) {
        return Err(ValidationError::new(
            "email_contains_invalid_special_characters",
        ));
    }
    ...
    if !domain_part
        .chars()
        .any(|c| valid_special_chars.contains(&c))
    {
        return Err(ValidationError::new(
            "email_missing_special_character_in_domain",
        ));
    }
    ...
    if !valid_domains
        .iter()
        .any(|&valid_domain| domain_part.ends_with(valid_domain))
    {
