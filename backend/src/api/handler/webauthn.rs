...
use crate::api::middlewares::webauthn_middleware::UserData;
use crate::errors::webauthn_error::{Error, WebResult};
...
pub(crate) async fn start_register(
    ...
    info_log!("Start register");
    info_log!("username = {}", username);
    info_log!("webauthn = {:#?}", webauthn);
    ...
    info_log!("user_unique_id = {}", user_unique_id);
    ...
    info_log!("exclude_credentials = {:?}", exclude_credentials);
    ...
    if let Err(err) = session.insert("reg_state", (username.as_str(), user_unique_id, reg_state)) {
        error_log!("Failed to save reg_state to session storage! err = {}", err);
        return Err(Error::SessionInsert(err));
    };

    info_log!("Registration Successful!");
    Ok(Json(ccr))
}
...
pub(crate) async fn finish_register(
    ...
    info_log!("Start finish_register");
