...
import { check, edit, star, trash } from "@/utils/Icons";
...
function TaskItem({ task }: TaskItemProps) {
  ...
  return (
    <motion.div
      className="h-[16rem] px-4 py-3 flex flex-col gap-4 shadow-sm bg-[#000D0D] text-white rounded-lg border-2 border-white"
      variants={item}
    >
      <div>
        <h4 className="font-bold text-2xl text-[#EDEDED]">{task.title}</h4>
        <p className="text-[#EDEDED]">{task.description}</p>
      </div>
